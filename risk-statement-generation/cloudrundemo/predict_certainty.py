...
class CertaintyEstimator(object):
    def __init__(self, task = 'sentence-level', cuda = False, use_auth_token=False):
        ...
        #ZP EDit for local model ref 2021-11-18
        sentLevPath = os.path.dirname(__file__) + '/pedropei/sentence-level-certainty'
        aspLevPath = os.path.dirname(__file__) + '/pedropei/aspect-level-certainty'
        
        
        if task == 'sentence-level':
            #model_path = 'pedropei/sentence-level-certainty'
            model_path = os.path.dirname(__file__) + '/pedropei/sentence-level-certainty'
            self.tokenizer = AutoTokenizer.from_pretrained(model_path, num_labels=1, output_attentions=False,
                                                         output_hidden_states=False, cache_dir = './model_cache', use_auth_token=use_auth_token,local_files_only=True)
            self.model = AutoModelForSequenceClassification.from_pretrained(model_path, num_labels=1,
                                     output_attentions=False, output_hidden_states=False,cache_dir = './model_cache',use_auth_token=use_auth_token,local_files_only=True)
        elif task == 'aspect-level':
            #model_path = 'pedropei/aspect-level-certainty'
            model_path = os.path.dirname(__file__) + '/pedropei/aspect-level-certainty'
